...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct 22 14:31:28 2024
...
def send_message_subdag(parent_dag_name, child_dag_name, start_date, schedule_interval, row):
    ...
    def subdag():
        ...
        def after_template():
            client = Client(TWILIO_SID, TWILIO_AUTH_TOKEN)
            fulano, dital, _, _, motivo, obs = row
            mess = f"""
Boa tarde {fulano}!
Estou passando para te lembrar da visita ao {dital}.
O motivo pelo qual não efetuou o pedido foi: {motivo}
E seu registro: {obs}
            """
            message = client.messages.create(
                body=mess,
...
for i in range(len(rows)):
    ...
    subdag_task = SubDagOperator(
        ...
        subdag=send_message_subdag('psg_5', f'conversation_{i}', datetime(2024, 10, 20), '35 20 * * *', row),
        dag=dag_one
    )
